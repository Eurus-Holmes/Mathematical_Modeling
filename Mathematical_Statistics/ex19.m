...
[beta,betaint,r,rint,st]=regress(Y,X) %计算回归系数和统计量等，st的第2个分量就是F统计量

%下面根据统计量的表达式重新计算的结果和这里是一样的
q=sum(r.^2) %计算残差平方和
